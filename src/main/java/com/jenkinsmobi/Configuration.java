...
public class Configuration {
  ...
  private static final String JENKINSCLOUD_URL =
      "http://jenkinscloud.com:9446/core";
  private static final String JENKINSCLOUD_SECRET =
  ...
  private static final String PRODUCT_NAME = "JenkinsMobi SDK";
  private static final String PRODUCT_VERSION = "1.0";
  public final String apkFileMD5;
  ...
  public void load() {
    ...
    username =
        prefs.getString(Configuration.KEY_USERNAME, JENKINSCLOUD_USERNAME);
    password =
        prefs.getString(Configuration.KEY_PASSWORD, JENKINSCLOUD_PASSWORD);
    // hudsonHostname = prefs.getString(Configuration.KEY_SERVICE_HOSTNAME,
    // JENKINSCLOUD_URL);
    ...
    userAgent =
        new WebView(appContext).getSettings().getUserAgentString()
            + " APK-MD5/" + apkFileMD5 + " Package/" + packageName;


    jenkinsUrl =
        prefs.getString(Configuration.KEY_JENKINS_URL,
            "http://hudson-mobi.com/hudson");
    jenkinsUsername =
        prefs.getString(Configuration.KEY_JENKINS_USERNAME, "guest");
    jenkinsPassword =
        prefs.getString(Configuration.KEY_JENKINS_PASSWORD, "guest");

    Logger.getInstance().info(
        String.format("PackageName:%s APK:%s",
            appContext.getApplicationInfo().packageName, apkFileMD5));
  }
  ...
  public boolean detectSubscriberId() {
    ...
    if (deviceId == null || deviceId.length() <= 0) {
      WifiManager wifiManager =
          (WifiManager) this.appContext.getSystemService(Context.WIFI_SERVICE);
    ...
    UUID deviceUuid =
        UUID.nameUUIDFromBytes((deviceId + subscriberId).getBytes());
